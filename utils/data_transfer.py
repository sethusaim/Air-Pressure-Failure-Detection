...
class Data_Transfer:
    def __init__(self):
        ...
        self.raw_data_bucket_name = self.config["s3_bucket"]["air_pressure_raw_data_bucket"]
    ...
    def transfer_mongo_raw_data_to_s3_bucket(self):
        ...
        try:
            ...
            self.s3.upload_folder(
                self.folder_name, self.raw_data_bucket_name, self.log_file, remove_folder=True
            )
    ...
    def split_raw_data(self):
        try:            
            files = self.s3.get_files_from_folder(self.folder_name,self.raw_data_bucket_name,self.log_file)
